<script lang="ts">
	...
	function displayQuestion(Ques: Question) {
		if (activeID != -1) {
			var currentElement = document.getElementById('question' + activeID)?.classList.remove('active');
		}

		activeID = Ques.questionNumber;

		document.getElementById('question' + Ques.questionNumber)?.classList.add('active');
		...
		<div class="row">
			...
			<div class="question-container col-md-6">
				<div class="questions-entries">
					<h1 contenteditable="true" bind:innerHTML={questionListHeading} class="display-4" />
					<p class="filler">Create a quiz now !</p>
					<div class="list-group">
						{#each Questions as ques}
							<button on:click|preventDefault={() => displayQuestion(ques)} id={'question' + ques.questionNumber} type="button" class="list-group-item list-group-item-action">{ques.question}</button>
						{/each}
					</div>
				</div>
			...
			<div class="quiz-form row col-md-6">
				<form method="get" class="row g-3 question-form">
					...
					<div class="col-12">
						...
						<input bind:value={question} type="text" class="form-control" id="questionTitle" placeholder="Enter your question here..." />
					</div>
					...
					<div class="btn-group col-12 row">
						<button on:click|preventDefault={addQuestion} type="submit" class="btn btn-primary col-sm-6">Add Question</button>
						<button on:click|preventDefault={clearQuestion} type="submit" class="btn btn-primary col-sm-6">Clear Question</button>
						<button on:click|preventDefault={deleteQuestion} type="submit" class="btn btn-primary btn-block">Delete This Question</button>
						<button on:click|preventDefault={saveChanges} type="submit" class="btn btn-primary col-sm-6">Save Changes</button>
					</div>
				</form>
			</div>
		</div>
